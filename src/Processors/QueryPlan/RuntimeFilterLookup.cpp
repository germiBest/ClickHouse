...
#    error "RuntimeFilterLookup requires xxHash >= 0.8.2 with XXH3 enabled (XXH3_128bits_withSeed)."
#endif
    ...
    BloomFilterHashPair * out_hashes)
{
    ...
    {
        out_hashes[row] = makeXXH3HashPair(position, value_size, seed);
        position += value_size;
    }
}

template <typename ProcessBatch>
void forEachColumnHashBatchXXH3(const IColumn & column, UInt64 seed, ProcessBatch && process_batch)
{
    ...
    if (!isColumnConst(column) && column.isFixedAndContiguous())
    {
        ...
        while (start_row < row_count)
        {
            ...
            hashFixedSizeXXH3(batch_data, value_size, batch_size, seed, hash_pairs.data());
    ...
    while (start_row < row_count)
    {
        ...
        for (size_t index = 0; index < batch_size; ++index)
        {
            ...
            hash_pairs[index] = makeXXH3HashPair(value.data(), value.size(), seed);
        }
        process_batch(hash_pairs.data(), batch_size, start_row);
        start_row += batch_size;
    }
...
ColumnPtr ApproximateRuntimeFilter::findImpl(const ColumnWithTypeAndName & values) const
{
    ...
    if (bloom_filter)
    {
        ...
        forEachColumnHashBatchXXH3(*values.column, bloom_filter->getSeed(),
...
{
    forEachColumnHashBatchXXH3(*values, bloom_filter->getSeed(),
        [&](const BloomFilterHashPair * hash_pairs, size_t count, size_t /* start_row */)
        {
