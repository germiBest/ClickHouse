...
void ColumnSparse::takeOrCalculateStatisticsFrom(const IColumn & source_column)
{
    values->takeOrCalculateStatisticsFrom(assert_cast<const ColumnSparse &>(source_column).getValuesColumn());
}
