...
void AlterCommand::apply(StorageInMemoryMetadata & metadata, ContextPtr context) const
{
    ...
    else if (type == MODIFY_ORDER_BY)
    {
        ...
        if (primary_key.definition_ast == nullptr && sorting_key.definition_ast != nullptr)
        {
            ...
            /// key.
            primary_key = KeyDescription::getKeyFromAST(sorting_key.definition_ast, metadata.columns, sorting_key.virtual_columns, context);
        }
...
void AlterCommands::apply(StorageInMemoryMetadata & metadata, ContextPtr context) const
{
    ...
    {
        metadata_copy.primary_key = KeyDescription::getKeyFromAST(metadata_copy.sorting_key.definition_ast, metadata_copy.columns, metadata_copy.sorting_key.virtual_columns, context);
